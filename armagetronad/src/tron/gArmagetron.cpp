...
#ifdef __ANDROID__
    // OUYA: the engine uses std file I/O, so point its data/config/var dirs at the
    // app's external files dir (writable; the AssetExporter unpacks the bundled game
    // data there on launch). Must run before tConfiguration loads anything.
    {
        const char *abase = SDL_AndroidGetExternalStoragePath();
        if ( abase && abase[0] )
        {
            tString base( abase );
            // Capture the engine's stdout/stderr to a file (Android drops them);
            // lets us read startup errors via adb pull.
            tString logPath = base + "/arma.log";
            freopen( logPath.c_str(), "w", stdout );
            freopen( logPath.c_str(), "a", stderr );
            setvbuf( stdout, NULL, _IONBF, 0 );
            setvbuf( stderr, NULL, _IONBF, 0 );
            tDirectories::SetData( base );
            tDirectories::SetUserData( base );
        }
