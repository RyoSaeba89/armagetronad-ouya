...
public:
    ...
    uJoystick( int id_ ):id(id_)
    {
...
#ifdef __ANDROID__
        // Use a stable, controller-name-independent persistent ID prefix so the
        // bundled OUYA config can bind the pad ("JOYSTICK_1_HAT_0_LEFT", etc.)
        // without depending on the exact SDL device name string.
        {
            std::ostringstream fixedName;
            fixedName << "JOYSTICK_" << ( id + 1 );
            internalName = fixedName.str();
        }
#endif
        ...
        __android_log_print( ANDROID_LOG_INFO, "ARMA-INPUT",
            "JOY id=%d name='%s' axes=%d buttons=%d hats=%d balls=%d",
            id, name.c_str(), numAxes, numButtons, numHats, numBalls );
#endif
...
public:
    uJoystickInput()
    {
        ...
        __android_log_print( ANDROID_LOG_INFO, "ARMA-INPUT", "su_GetJoystickInput: SDL_NumJoysticks()=%d", numJoysticks );
...
{
    // NOTE: the OUYA controller->keyboard bridge lives in su_GetSDLInput
    // (uInputQueue.cpp), at the central event fetch point, so it also reaches the
    // menu / message-box loops that bypass this function.
    switch (e.type)
    {
