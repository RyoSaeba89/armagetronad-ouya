...
bool su_GetMenuInput(SDL_Event &tEvent,REAL &time); //!< like su_GetSDLInput, but maps pad events to menu nav keys (Android)
...
inline bool su_GetMenuInput(SDL_Event &tEvent){
    REAL dummy;
    return su_GetMenuInput(tEvent,dummy);
}
