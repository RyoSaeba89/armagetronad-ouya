...
{
#ifdef __ANDROID__
    // OUYA/Tegra 3: the window surface is RGB_565. Force a MATCHING ES2 config
    // (RGB 5/6/5 + 16-bit depth); requesting 8/8/8 finds no compatible EGLConfig, so
    // eglChooseConfig fails -> the engine loops retrying video modes and freezes input.
    // Request the ES2 context BEFORE window creation (else EGL_BAD_DISPLAY).
    rDepth = 5; gDepth = 6; bDepth = 5; zDepth = 16;
...
#ifndef __ANDROID__
        // On Android SDL_StartTextInput() pops the on-screen keyboard, which steals
        // focus from the gamepad. Key events still flow without it (menus use keydown).
        SDL_StartTextInput();
#endif
        break;
    case 0:
...
void sr_LoadDefaultConfig(){
    ...
    */
...
#ifdef __ANDROID__
    // OUYA / Tegra 3 is GPU fill-bound. The NVIDIA branch above leaves the floor at
    // rFLOOR_TWOTEXTURE (full-arena textured grid = the single biggest overdraw cost).
    // This function is also re-run on FIRST_USE (gArmagetron.cpp), which is exactly the
    ...
    sr_dither        = false;         // no dithering pass
    sr_texturesTruecolor = false;     // 16-bit textures: less bandwidth
    sr_infinityPlane = false;
#endif
}
